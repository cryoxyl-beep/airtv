...
navbar = navbar.replace(
  ...
  '<button\\n                type="button"\\n                onClick={() => setIsFilterOpen(!isFilterOpen)}\\n                className={`flex items-center justify-center h-8 md:h-9 px-3 md:px-4 rounded-full text-xs md:text-sm font-medium transition-colors border ${isFilterOpen ? \\'bg-white text-black border-white\\' : \\'bg-[#1A1A1A] border-white/20 text-white hover:bg-white/10 shadow-sm\\'}`}'
);
