...
https.get(options, (res) => {
  ...
  res.on('end', () => {
    ...
    const results = json.results.filter(p => p.provider_name.toLowerCase().includes('paramount'));
    console.log(JSON.stringify(results.map(p => ({id: p.provider_id, name: p.provider_name})), null, 2));
  });
});
